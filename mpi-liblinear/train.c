...
void exit_with_help()
{
	...
	"	 0 -- L2-regularized logistic regression primal (tron)\n"
	"	 1 -- L2-regularized L2-loss support vector classification (dual)\n"
	"	 2 -- L2-regularized L2-loss support vector classification primal (tron)\n"
	"	 3 -- L2-regularized L1-loss support vector classification (dual)\n"
	"	 8 -- L2-regularized logistic regression primal (l-commdir)\n"
	"	 9 -- L2-regularized L2-loss support vector classification primal (l-commdir)\n"
	...
	"	-s 1 and 3 \n"
	"		Relative duality gap <= eps (default 0.01)\n"
	"-B bias : if bias >= 0, instance x becomes [x; bias]; if < 0, no bias term added (default -1)\n"
	"-wi weight: weights adjust the parameter C of different classes (see README for details)\n"
	"-v n: n-fold cross validation mode\n"
	"-C : find parameter C (only for -s 0, 2, 8, and 9)\n"
	"-m states: number of history states for l-commdir (default 5)\n"
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	if(flag_find_C)
	{
		...
		else if(param.solver_type != L2R_LR && param.solver_type != L2R_L2LOSS_SVC && param.solver_type != L2R_LR_COMM && param.solver_type != L2R_L2LOSS_SVC_COMM)
		{
			fprintf(stderr, "Warm-start parameter search only available for -s 0, 2, 8, and 9\n");
			exit_with_help();
		}
	}
